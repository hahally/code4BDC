...
from scripts.trick import EMA,FGM, compute_kl_loss
...
class Config():
    ...
    vocab_path = './pretrain_model/DAE_seq2seq_large_last/vocab.txt'
    model_path = '../预训练/pretrain_model/bart-base'
    checkpoint = None
    save_model_path = './saved_models/bart-base-rdrop'
...
def build_optim(config, model):
    ...
    optimizer_parameters = [
        ...
        {'params':[p for n,p in params if any(nd in n for nd in other) and any(nd in n for nd in no_decay) ],'weight_decay':0,'lr':2e-3},
        {'params':[p for n,p in params if any(nd in n for nd in other) and not any(nd in n for nd in no_decay) ],'weight_decay':1e-2,'lr':2e-3},
    ]
...
def train_epoch(config):
    ...
    fgm = FGM(model,epsilon=1.0, emb_name='shared')
    ...
    for epoch in range(config.train_epoch):
        for data in tqdm(train_loader):
            ...
            ids = data['source_ids'].to(device, dtype = torch.long)
            mask = data['source_mask'].to(device, dtype = torch.long)
            out_1 = model(input_ids = ids, attention_mask = mask, decoder_input_ids=y_ids, labels=lm_labels)
            out_2 = model(input_ids = ids, attention_mask = mask, decoder_input_ids=y_ids, labels=lm_labels)
            pad_mask = lm_labels==tokenizer.pad_token_id
            pad_mask = pad_mask.unsqueeze(-1)
            kl_loss = compute_kl_loss(out_1.logits, out_2.logits, pad_mask=pad_mask) * 10
            ce_loss = (out_1.loss.mean() + out_2.loss.mean()) * 0.5
            ...
            if step%config.print_step==0:
                ...
                info = {
                    ...
                    'ce_loss':batch_ce_loss/config.print_step,
                    'kl_loss':batch_kl_loss/config.print_step,
                    'lr':lr
                }
                print(info)
                batch_loss = 0
            ...
            if step % eval_step==0:
                ...
                if bst_score<score:
                    bst_score = score
                    save_model(model, tokenizer, config.save_model_path)
