...
class Config():
    ...
    model_path = './pretrain_model/DAE_seq2seq_large_last'
    checkpoint = None
    save_model_path = './saved_models/bart-base-sparsemax'
...
def train_epoch(config):
    ...
    loss_fct = Sparsemax(k_sparse=20)
    bst_score = 0
    ...
    for epoch in range(config.train_epoch):
        for data in tqdm(train_loader):
            ...
            if step%config.print_step==0:
                ...
                print(f'Epoch: {epoch+1}, step:{step}, Loss:  {batch_loss/config.print_step}, lr:{lr}')
                batch_loss = 0
