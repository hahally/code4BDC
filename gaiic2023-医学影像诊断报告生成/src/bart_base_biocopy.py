...
from scripts.BIOCopymodel import BartForConditionalGenerationWithBioCopy
from transformers.optimization import get_cosine_schedule_with_warmup
import torch
from torch.optim import AdamW
import os
import pandas as pd
from scripts.dataset import create_BIO4BART_dataset
...
class Config():
    ...
    vocab_path = './pretrain_model/DAE_seq2seq_large/large_vocab.txt'
    model_path = './pretrain_model/DAE_seq2seq_large'
    checkpoint = None
    save_model_path = './saved_models/bart-base-biocopy-fgm'
...
def build_model(config):
    ...
    model = BartForConditionalGenerationWithBioCopy.from_pretrained(config.model_path)
...
def build_optim(config, model):
    ...
    other = ['enc_bio_fc','dec_bio_fc']
    no_main = no_decay + other
    params = list(model.named_parameters())
    optimizer_parameters = [
        {'params':[p for n,p in params if not any(nd in n for nd in no_main)],'weight_decay':1e-2,'lr':config.lr},
        {'params':[p for n,p in params if not any(nd in n for nd in other) and any(nd in n for nd in no_decay) ],'weight_decay':0,'lr':config.lr},
        {'params':[p for n,p in params if any(nd in n for nd in other) and any(nd in n for nd in no_decay) ],'weight_decay':0,'lr':1e-3},
        {'params':[p for n,p in params if any(nd in n for nd in other) and not any(nd in n for nd in no_decay) ],'weight_decay':1e-2,'lr':1e-3},
    ]
    optimizer = AdamW(params =  optimizer_parameters, betas=(0.9,0.95), lr=config.lr, eps=1e-7)
...
def train_epoch(config):
    ...
    train_loader = create_BIO4BART_dataset(src=train_dataset[1].tolist(),
                                    ...
                                    pin_memory=True,
                                    num_workers=0)
    valid_loader = create_BIO4BART_dataset(src=val_dataset[1].tolist(),
    ...
    config.print_step = config.print_step if config.print_step else len(train_loader) #2000
    step = 0
    bst_score = 0
    batch_loss = 0
    batch_lm_loss = 0
    ...
    for epoch in range(config.train_epoch):
        for data in tqdm(train_loader):
            ...
            model.train()
            optimizer.zero_grad()
            src_labels = data['src_label'].to(device, dtype = torch.long)
            tgt_labels = data['tgt_label'].to(device, dtype = torch.long)
            y = data['target_ids'].to(device, dtype = torch.long)
            ...
            ids = data['source_ids'].to(device, dtype = torch.long)
            mask = data['source_mask'].to(device, dtype = torch.long)
            out = model(src_labels=src_labels[:, 1:].clone().detach(),
                        tgt_labels=tgt_labels[:, 1:].clone().detach(),
                        input_ids=ids,
                        attention_mask=mask,
                        decoder_input_ids=y_ids,
                        labels=lm_labels)
            
            lm_loss = out.loss.mean()
            bio_loss = (out.enc_bio_loss.mean() + out.dec_bio_loss.mean()) * 2
            loss = lm_loss + bio_loss
            batch_loss += loss.item()
            batch_lm_loss += lm_loss.item()
            batch_bio_loss += bio_loss.item()
            batch_src_acc += out.src_acc.item()
            batch_tgt_acc += out.tgt_acc.item()
            loss.backward()
            ...
            if step%config.print_step==0:
                ...
                info = {"epoch":epoch+1,
                        "step":step,
                        "Loss":batch_loss/config.print_step,
                        "lm_loss":batch_lm_loss/config.print_step,
                        "bio_loss":batch_bio_loss/config.print_step,
                        "src_acc":batch_src_acc/config.print_step,
                        "tgt_acc":batch_tgt_acc/config.print_step,
                        "lr":lr
                        }
                print(info)
                batch_loss = 0
            ...
            # if step % eval_step==0 and epoch+1>config.train_epoch//2:
            if step % eval_step==0 and epoch>10:
                lr = optimizer.state_dict()['param_groups'][0]['lr']
                if epoch+1>start_ema:
                    ema.apply_shadow()
