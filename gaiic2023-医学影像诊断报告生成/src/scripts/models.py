...
class BartForConditionalGenerationWithCopyMech(BartForConditionalGeneration):
    def __init__(self, config: BartConfig):
        ...
        self.copy_module = CopyMechModule(config.d_model, 1356)
        self.post_init()
    ...
    def forward(
        ...
        if labels is not None:
            ...
            p_gen, cp_logits = self.copy_module(input_ids, cross_attentions, outputs.encoder_last_hidden_state, hidden_states)
            p_copy = 1 - p_gen
            logits = p_gen * lm_logits + p_copy * cp_logits
        else:
            # Inference (Step-by-step Decoding)
            cross_attentions = outputs.cross_attentions[-1].mean(dim=1)
            p_gen, cp_logits = self.copy_module(src_input_ids, cross_attentions, encoder_outputs[0], hidden_states)
            p_copy = 1 - p_gen
            logits = p_gen * lm_logits + p_copy * cp_logits
        ...
        if labels is not None:
            ...
            # loss_fct = Sparsemax(k_sparse=50)
            # labels[labels == -100] = 0
